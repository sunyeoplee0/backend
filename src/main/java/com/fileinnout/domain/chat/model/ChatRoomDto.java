...
public class ChatRoomDto {
    ...
    public record CreateRequest(        // 채팅방 생성 요청 DTO
            String title,               // 채팅방 이름
            List<Long> participantIdx   // 채팅 초대 대상자
    ) {}
    public record ChatRoomCreateResponse(   // 채팅방 생성 응답 DTO
            Long roomId,                    // 채팅방 번호
            String title,                   // 채팅방 이름
            LocalDateTime createdAt         // 채팅방 생성시간
    ) {}

    public record ChatRoomListRequest(      //채팅방 목록 조회 요청
            int page,
            int size
    ) {}

    public record ChatRoomListResponse(     //채팅방 목록 조회 응답
            Long roomId,          // 채팅방 번호
            String title,         // 채팅방 이름
